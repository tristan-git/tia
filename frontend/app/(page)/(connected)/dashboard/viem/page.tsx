...
const client = createPublicClient({
	chain: mainnet,
	transport: http('https://testnet.skalenodes.com/v1/giant-half-dual-testnet'),
})
	...
	useEffect(() => {
		...
		const getInfos = async () => {
			const logs = await client.getLogs({
				address: '0xc8c6938A25F4726C869a252831508B24Ad84b78C',
				event: parseAbiItem('event VotingContractDeployed(address indexed contractAddress, string votingTitle)'),
				fromBlock: 2512723n,
				// toBlock: 15055228n,
			})
			//
					...
					<Alert key={crypto.randomUUID()}>
						...
						<AlertDescription>
							<p key={crypto.randomUUID()}>
								Adresse : {event?.args.contractAddress} - title : {event?.args.votingTitle}
							</p>
						</AlertDescription>
					</Alert>
				))}
